...
const Booking = () => {
  ...
  const handleDateChange = (daysToAdd) => {
    ...
    setEndDate(formattedDate); // Initialize end date to the same day by default
  };

  // Time slots generation (30 mins gap)
  ...
  const handleStartTimeChange = (e) => {
    ...
    if (!endTime) setEndDate(startDate); // Reset end date if endTime is not yet set
  };
  ...
  const handleEndTimeChange = (e) => {
    ...
    // If the end time is earlier than the start time, update endDate to the next day
    if (end < start) {
      const nextDay = new Date(start);
      nextDay.setDate(nextDay.getDate() + 1);
      setEndDate(nextDay.toISOString().split("T")[0]);
    } else {
      setEndDate(startDate); // Otherwise, keep end date the same as start date
    }
  };
